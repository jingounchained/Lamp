    ...
    internal static class FileHandler
    {
        ...
        private static async Task<bool> SafeAcquirePackageInMemory(string packageURL, string packageDestination)
        {
            ...
            try
            {
                ...
                if (packageURL.Contains("github.com"))
                {
                    string projectName = "", branchFolder ="";
                    string[] uriComponents = packageURL.Split("/", StringSplitOptions.RemoveEmptyEntries);
                    //index 3 should be the Project name (https://github.com/account/project/)
                    if(uriComponents.Length > 3 ) projectName = uriComponents[3]; 
                    branchFolder = projectName + "-" + System.IO.Path.GetFileNameWithoutExtension(packageURL) + "/";
                    if (archive.Entries.Count > 0 && archive.Entries[0].FullName.ToUpper() == branchFolder.ToUpper())
                    {
                        /*
                         * Our "supported" method of hosting a repo is to use the path to the main branch's 
                         * zip file on github (see the Paths) which will contain a root folder that is Projectname-Branch
                         * We want to remove that from the start of every entry. 
                         */

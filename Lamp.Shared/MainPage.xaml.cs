...
namespace Lamp
{
    ...
    public sealed partial class MainPage : Page, INotifyPropertyChanged
    {
        ...
        private ContentDialog confirmationDialog;
            ...
            await Task.Run(async () =>
            {
                ...
                switch (action)
                {
                    ...
                    case "scripts":
                        ...
                        MessageDialog dialog = new MessageDialog("This will OVERWRITE any existing Scripts you have with the same name! You should make sure you have them backed up first.", "Confirm Potential Overwrite");
                        WinRT.Interop.InitializeWithWindow.Initialize(dialog, CurrentApp.HWND);
                        dialog.Commands.Add(new UICommand("Understood.") { Id = 1 });
                        dialog.Commands.Add(new UICommand("Wait, no!") { Id = 0 });
                        UICommand response = (UICommand)await dialog.ShowAsync();
                        if (response.Id == "1") 
                        { 
                            await ExecutionHandler.UpdateScripts(repo, dir);
                        }
                        break;
                }
